...
import { Variable, Visualizer } from '../types';
...
export class JSONViewer implements Visualizer {
    context: vscode.ExtensionContext;
    value: Variable;

    constructor(context: vscode.ExtensionContext, variable: Variable) {
        this.context = context;
        this.value = this.preprocess(variable);
    }

    canHandle(variable: Variable): boolean {
        // Check if the variable value is a string that looks like JSON
        if (typeof variable.value === 'string') {
            const likelyJson = this.isLikelyJson(variable.value);
            return likelyJson.isLikely;
        }
        // If it's not a string, we can't handle it
        return false;
    }
     ...
     */
    isLikelyJson(str: string): { isLikely: boolean, reason?: string } {
     ...
     */
    preprocessJsonString(jsonValue: string): string {
     ...
     */
    getDetailedJsonErrorMessage(error: Error, jsonValue: string, variableName: string): string {
    ...
    preprocess({name, value}: Variable): any {
                const processedValue = this.preprocessJsonString(value);
    
                // Check if the value is likely JSON before attempting to parse
                const likelyJson = this.isLikelyJson(processedValue);
                if (!likelyJson.isLikely) {
                    vscode.window.showErrorMessage(`"${name}" does not appear to be valid JSON: ${likelyJson.reason}`);
                    return;
                }
                ...
                // Create and show the JSON viewer
                    ...
                    if (error instanceof Error) {
                        const detailedError = this.getDetailedJsonErrorMessage(error, processedValue, name);
                        vscode.window.showErrorMessage(detailedError, { modal: true });
                    } else {
                        vscode.window.showErrorMessage(`"${name}" does not appear to be valid JSON`);
                    }
                }
    }
    ...
    visualize(webview: vscode.Webview): string {
        ...
        const jsonEditorJsUri = webviewUri(webview, extensionUri, ['node_modules', 'jsoneditor', 'dist', 'jsoneditor.min.js']);
        const jsonEditorCssUri = webviewUri(webview, extensionUri, ['node_modules', 'jsoneditor', 'dist', 'jsoneditor.min.css']);

        // Set the HTML content
        return getWebviewContent(this.value, webview, jsonEditorJsUri, jsonEditorCssUri);
    }
